...
const Flow = () => {
  ...
  const onNodeDrag = useCallback((_: MouseEvent, draggedNode: Node) => {
    const intersections = getIntersectingNodes(draggedNode).map((n) => n.id);

    setNodes((currentNodes) =>
      currentNodes.map((node) => ({
        ...node,
        className:  // add the "highlight-green" class to the intersection node, if another node is dragged onto it
  ...
  const generateImageNode = async (prompt: string = "bunny on the moon") => { //bunny on the moon by default
  ...
  return (
    <>
      ...
      <button onClick={() => generateImageNode("kawaii bunny with a witch hat")}> Generate an Image</button>
        <ReactFlow
          nodes={nodes}
          nodeTypes={nodeTypes}
